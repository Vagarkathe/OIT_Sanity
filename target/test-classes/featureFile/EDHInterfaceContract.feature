...
Feature: Interface Contract API Sanity
  ...
  Scenario: Histories Dataset
   Given Histories dataset API
   Then The streaming interface contract history should be displayed
   
   Scenario: Histories Dataset add Contract Version History 4
   Given Histories dataset API add contract version
   Then The contract version histort should be added
   
   Scenario: Update Contract Version History 
   Given Interfacecontract update Contract Version 
   Then History should be updated
   
   Scenario: Interface Contract Update Dataset
   Given Interfacecontract update datset 
   Then Datset should be updated
   
   Scenario: Interface Contract Delete Dataset
   Given Interfacecontract delete datset 
   Then Datset should be delete
   
   Scenario: Interface Contract Search Dataset
   Given Interfacecontract search datset 
   Then Datset should be search
   
   Scenario: Interface Contract Upload XLS
   Given Interfacecontract upload the XLS
   Then Version should be update
   
   Scenario: Interface Contract Upload JSON
   Given Interfacecontract upload the json
   Then JOSN Version should be update
   
   Scenario: Interface Contract Upload Multiple JSON
   Given Interfacecontract upload the multiple json
   Then Multiple JOSN Version should be update
