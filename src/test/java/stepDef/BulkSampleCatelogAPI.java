...
public class BulkSampleCatelogAPI {
	RequestSpecification request_spec;
	Response response;
	
	String baseUrl = "https://portal-int-edh.bigdata.intraorange/api";
	File BulkCATELOGjson = new File("src/test/resources/Payloads/BulkCatelog.json");
	Scenario scn;

	@Given("Send the bulk sample catelog API to server")
	public void send_the_bulk_sample_catelog_api_to_server() {
		RestAssured.baseURI= baseUrl+ "/v2/catalog/";
		RestAssured.basePath="{environment}/samplecatalog/bulk";
		request_spec = given().baseUri(RestAssured.baseURI)
			...
			//	.pathParam("datasourceId", "569")
		        ...
		        .body(BulkCATELOGjson);
	}

	@Then("Validate the response code as {int} OK and get the response")
	public void validate_the_response_code_as_ok_and_get_the_response(Integer int1) {
