...
public class RemoveContact {
	...
	public void send_the_remove_contact_api_to_server() {
		RestAssured.baseURI= baseUrl+ "/v2/catalog/";
		RestAssured.basePath="{environment}/contact/{contactId}";
		request_spec = given().baseUri(RestAssured.baseURI)
				.pathParams("environment", "INT","contactId","9")
				.basePath(RestAssured.basePath);
		scn.log("Requested URL: " + baseURI + basePath);
	}

	@Then("Validate status code as {int} and check the response")
	public void validate_status_code_as_and_check_the_response(Integer int1) {
