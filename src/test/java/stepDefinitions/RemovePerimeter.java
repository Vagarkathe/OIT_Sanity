...
public class RemovePerimeter {
	...
	public void send_the_remove_perimeter_api_to_server() {
		RestAssured.baseURI= baseUrl+ "/v2/catalog/";
		RestAssured.basePath="{environment}/perimeter/{perimeterId}";
		request_spec = given().baseUri(RestAssured.baseURI)
				.pathParams("environment", "INT","perimeterId","318")
				.basePath(RestAssured.basePath);
		scn.log("Requested URL: " + baseURI + basePath);
	}

	@Then("Validate the response code as {int} and get the response")
	public void validate_the_response_code_as_and_get_the_response(Integer int1) {
