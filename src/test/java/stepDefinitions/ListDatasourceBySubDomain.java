...
public class ListDatasourceBySubDomain {
	...
	public void endpoints_of_list_data_sources_by_subdomain_api_are_sent_to_server() {
		RestAssured.baseURI= baseUrl+ "/v2/catalog/";
		RestAssured.basePath="{environment}/datasources/subdomain/{subDomain}";
		request_spec = given().baseUri(RestAssured.baseURI)
				.pathParams("environment", "INT", "subDomain", "To be defined")
				.basePath(RestAssured.basePath);
		scn.log("Requested URL: " + baseURI + basePath);
	}

	@Then("Fetch the response and validate response contains {int}")
	public void fetch_the_response_and_validate_response_contains(Integer int1) {
