...
public class SaveDoc {
	...
	File Savedocjson = new File("src/test/resources/Payloads/saveDoc.json");
	
Scenario scn;
	
	@Given("Hit the Save Doc API with its headers and parameters")
	public void hit_the_save_doc_api_with_its_headers_and_parameters() {
		RestAssured.baseURI= baseUrl+ "/v2/catalog/";
		RestAssured.basePath="{environment}/datasource/{datasource}/doc";
	...
	@Then("Fetch the response and validate {int} OK as status code")
	public void fetch_the_response_and_validate_ok_as_status_code(Integer int1) {
